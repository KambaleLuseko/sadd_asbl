...
if ($transaction == 'news') {
    ...
    if (isset($data['image']) && isset($data['image2'])) {
        ...
        // if(isset($video) && isset($decodedVideo)){
    ...
    $sql = "INSERT INTO realisations (textes, auteur, image, image2, date_pub, contenu".(isset($video) ? ', video' : "").") 
            VALUES ('$textes', '$auteur', '$upload_path1', '$upload_path2', '".date('Y-m-d H:i:s')."', '$contenu'".(isset($video) ? ", '$video'" : "").");";
    } catch (\Throwable $th) {
        http_response_code(500);
